...
- Guard `DialogueManager.run_node("")` — empty string throws a C# YarnSpinner exception (generic behavior: shared wiki, `godot-yarnspinner.md`). Always `if dialogue_node == "": return`
...
- **`run_node()` with an uncompiled dialogue_node crashes the test, not just logs an error (confirmed empirically, issue #93 Task 3):** `_fire()` calling `DialogueManager.run_node(id)` for an `id` not in any `dialogue/*.yarn` file throws `Yarn.DialogueException` (generic exception / GUT Unexpected-Errors-gate / never-await-after behavior: shared wiki, `godot-yarnspinner.md`)
...
> Engine-generic rules promoted to the shared wiki: C:\Code\knowledge\godot-yarnspinner.md ([YarnCommand] registration, unknown-command halt, DialogueException on bad node ids, $-prefix normalization rule)
...
- `GameStateVariableStorage.NormalizeKey` implements the shared wiki's `$`-prefix normalization rule across all four VariableStorageBehaviour overrides (GameState stores keys without the `$`)
