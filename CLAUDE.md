...
- Maps: Tiled → YATI importer (`addons/YATI`); tileset PNG at `assets/tilesets/`
  ...
  - Reimport after changing a TMX: `godot_console --headless --editor --quit --path .`
...
- After editing any `.yarn` file, delete the compiled yarnproject cache and reimport before running:
  ```bash
  rm -f .godot/imported/iris.yarnproject-*.tres .godot/imported/iris.yarnproject-*.md5
  godot_console --headless --editor --quit --path .
  ```
  The `/run` skill does this automatically when `.yarn` files are newer than the compiled `.tres`.
